<template>
    <div>
        <div class="bg-slate-100 dark:bg-slate-700 dark:bg-slate-900">
        <div class="main-container py-4">
            ...
            <!-- <div class="p-3 xl:p-6 flex-col sm:flex-row justify-center items-center gap-1 sm:gap-8 flex rounded-xl mb-3"
                ...
                <div class="flex justify-center items-center gap-2 xl:gap-4">
                    ...
                    <div class="flex justify-center items-center gap-2 text-white">
                        <div class="w-8 h-8 sm:w-11 sm:h-11 bg-white dark:bg-slate-800 rounded-full justify-center items-center flex">
            ...
            <div v-if="!isLoading" class="w-full p-2 sm:p-4 bg-white dark:bg-slate-800 rounded-lg sm:rounded-xl md:rounded-2xl flex gap-3 md:gap-6 items-center justify-between">
                <!-- Back -->
                <router-link to="/" class="py-2 flex gap-1 sm:gap-2 items-center justify-center">
                    <ArrowLeftIcon class="w-4 h-4 sm:w-6 sm:h-6 text-slate-600 dark:text-slate-400" />
                    <div class="text-slate-600 dark:text-slate-400 text-sm sm:text-base font-normal leading-normal">
                        {{ $t("Back") }}
                    </div>
                </router-link>
                    ...
                    <swiper :slidesPerView="'auto'" :spaceBetween="16" class="categorySwiper">
                        ...
                        <swiper-slide>
                            <div class="p-2 sm:px-4 sm:py-3 rounded-md sm:rounded-[10px] border text-base font-normal leading-normal hover:text-primary cursor-pointer transition duration-300"
                                :class="!route.query.subcategory ? 'text-primary border-primary' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'"
                        ...
                        <swiper-slide v-for="subcategory in subcategories" :key="subcategory.id">
                            <div class="p-2 sm:px-4 sm:py-3 rounded-md sm:rounded-[10px] border text-base font-normal leading-normal hover:text-primary cursor-pointer transition duration-300"
                                :class="(subcategory.slug == route.query.subcategory) ? 'text-primary border-primary' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'"
                                @click="$router.push(`/categories/${route.params.slug}?subcategory=${subcategory.slug}`)">
                                {{ subcategory.name }}
                            </div>
                        </swiper-slide>
                ...
                <div>
                    <button
                        ...
                        :class="hasFilter ? ' bg-primary-200 text-primary' : 'text-slate-600 dark:text-slate-400 bg-slate-200'"
            ...
            <!-- loading -->
            <div v-else class="w-full p-2 md:p-4 bg-white dark:bg-slate-800 rounded-lg sm:rounded-xl md:rounded-2xl flex gap-3 md:gap-6 items-center justify-between">
                        ...
                        <div class="pointer-events-none fixed inset-y-0 flex max-w-full" :class="master.langDirection == 'rtl' ? 'left-0 sm:pr-10' : 'right-0 sm:pl-10'">
                            <TransitionChild as="template"
                                ...
                                <DialogPanel class="pointer-events-auto relative w-screen max-w-md">
                                    ...
                                    <div
                                        class="flex h-full flex-col justify-between overflow-y-scroll bg-white dark:bg-slate-800 shadow-xl">
                                        <div class="p-4 flex flex-col gap-7">
                                            ...
                                            <!-- Customer Review -->
                                            <div>
                                                <div class="text-slate-950 dark:text-white text-base font-medium leading-normal">
                                                    {{ $t("Customer Review") }}
                                                </div>
                                                <!-- Rating -->
                                                <div class="flex flex-col gap-2 mt-3">
                                                    <div v-for="rating in ratings" :key="rating" class="grow">
                                                        <label :for="`rating${rating}`"
                                                            class="cursor-pointer has-[:checked]:border-primary text-slate-800 dark:text-slate-100 flex items-center justify-between px-2 py-1.5 bg-white dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700 gap-1.5">
                                            ...
                                            <div>
                                                ...
                                                <div
                                                    class="flex flex-wrap gap-4 p-3 rounded-xl border border-slate-200 dark:border-slate-700 mt-1">
                                                    <label v-for="color in filter.colors" :key="color.id"
                                                        class="cursor-pointer has-[:checked]:text-primary text-slate-800 dark:text-slate-100 flex items-center p-2 bg-white dark:bg-slate-800 gap-2">
                                        ...
                                        <div class="flex gap-6 p-6 border-t border-slate-200 dark:border-slate-700">
                                            <button
...
const fetchProducts = async () => {
    ...
    axios.get('/products', {
        params: {
            ...
            sub_category_slug: route.query.subcategory,
            ...filterFormData.value,
        },
        headers: {
...
const fetchSubCategories = async () => {
    axios.get('/sub-categories?category_slug=' + route.params.slug,{
        headers: {
            'Accept-Language': master.locale || 'en',
        }
    }).then((response) => {
