            ...
            <div class="card-body">
                <div class="widget_container col-span-6 rounded-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 p-3 mb-3">
                    ...
                    <div class="glassmorphism box-2">
                        <p class="fs-4 fw-semibold mb-3 price">{{ showCurrency(formatAmount($totalOrder->totalProfit)) }}
                        </p>
                        <div class="d-flex justify-content-between">
                        ...
                        <tbody>
                            @forelse($orders as $order)
                                <tr class="text-center">
                                    ...
                                    <td>{{ showCurrency(formatAmount($order->total_buying_amount)) }}</td>
                                    <td>{{ showCurrency(formatAmount($order->total_sale_amount)) }}</td>
                                    <td>{{ showCurrency(formatAmount($order->admin_commission)) }}</td>
                                    <td>{{ showCurrency(formatAmount($order->profit)) }}</td>
                                </tr>
                            @empty
                                <tr>
